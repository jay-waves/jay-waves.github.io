...
= 深度精简 Windows11 系统
Windows11 一直是我的主力机，随着近几次更新，感觉它越来越臃肿了：经常有莫名其妙的进程在大幅占用 CPU、内存， 完全空载的情况下 Windows 系统本身能吃掉我接近 15GB 内存。作为轻度洁癖，不能忍受微软这种喂粪行为，于是开始激进地精简我的电脑系统。

== TL;DR
- 删掉不用的系统软件：#link("https://github.com/Raphire/Win11Debloat")[Raphire/Win11Debloat]
- 删除所有 AI 功能（第三方总能找到替代品）：#link("https://github.com/zoicware/RemoveWindowsAI")[zoicware/RemoveWindowsAI]
- 删除 Defender，大部分情况无需杀毒软件，用防火墙与浏览器安全插件即可。#link("https://github.com/ionuttbara/windows-defender-remover")[ionuttbara/windows-defender-remover]
...
- #link("https://github.com/Raphire/Win11Debloat")[Raphire/Win11Debloat]
- #link("https://github.com/ChrisTitusTech/winutil")[ChrisTitusTech/winutil]
- #link("https://geekuninstaller.com/download")[Geek]
...
- Recall: 后台服务名叫 #strong[WorloadsSessionHost]，这个我真没招了，起名还以为是系统进程，占了 2GB 内存零作用。
- AI Fabric Service: 后台的服务名叫 #strong[WSAIFabricSvc]，某天我发现它莫名占用了大量 CPU。
- Click to Do
- Copilot, AI Host: 大陆用户默认不会运行，但是会占磁盘。

工具：#link("https://github.com/zoicware/RemoveWindowsAI")[zoicware/RemoveWindowsAI]
...
工具：#link("https://github.com/ionuttbara/windows-defender-remover")[ionuttbara/windows-defender-remover]
...
- #link("https://github.com/amymor/1Click-StartMenu")[anymor/1Click-StartMenu] 删掉 Windows 开始菜单
- #link("https://learn.microsoft.com/en-us/windows/powertoys/command-palette/overview")[powertoys/command-palette] 用这个作为类似 Mac Spotlight 的应用打开器。

开始菜单是一个受系统保护的 UWP 应用，因此 #link("https://github.com/amymor/1Click-StartMenu")[anymor/1Click-StartMenu] 脚本的实际功能是：修改 exe 属主为管理员群组，然后用一个空文件替代掉原本的 exe 文件。开始菜单（Start Menu）的文件位置大概是 `C:\Windows\SystemApps\Microsoft.Windows.StartMenuExperienceHost_cw5n1h2txyewy\`
...
- 禁用 Web SearchHost: #link("https://github.com/shoober420/windows11-scripts/blob/main/DisableSearchHost.bat")[windows11-scripts/DisableSearchHost.bat]

#strong[上文三个应用（开始、继续、搜索）的 UI 界面都是系统级 UWP 应用，禁用服务也不会停止后台进程，必须处理 exe 文件。即使删除后，每次 Windows 更新后也会被微软修复，不想折腾可以先不管。]
...
== 改改改
=== 清理右键菜单
首先，将右键菜单从 Win11 切换为 Win10，可以用 #link("https://github.com/valinet/explorerpatcher")[explorer patcher] 来做。Win11 的右键菜单实在是太卡了，强烈建议干掉。

Win10 右键菜单的问题是，垃圾软件经常往里塞东西，导致很臃肿。这里推荐用 #link("https://github.com/BluePointLilac/ContextMenuManager/tree/master")[ContextMenuManager] 来管理。

对于一些流氓软件，用 ContextMenuManager 干不掉的菜单项，让 AI 帮忙写注册表文件 `.reg` 来修改。

=== Shell 环境
推荐在 Windows 下使用 #link("https://github.com/powershell/powershell")[PowerShell7] + #link("https://github.com/microsoft/terminal")[Windows Terminal] 组合。#strong[用 CMD 会变得不幸]。

不推荐使用 MSYS2 等兼容 GNU 环境，需要具体命令可以用原生 Windows 替代品：

- #link("https://github.com/nscaife/file-windows")[file]
- #link("https://github.com/uutils/coreutils")[coreutils] 包含各类 GNU 常见工具
- 各类 Rust 重写工具：rg, bat, fd, dust

=== C++ 开发环境
C++ 只推荐使用 #link("https://visualstudio.microsoft.com/downloads/")[Build Tools for Visual Studio (MSVC)]，这是 Visual Studio 的后端环境，也是 Windows 原生的 C++ 后端。下载安装器后，挑选自己需要的组件安装即可，不要全部安装，共计 2.5GB 左右。
...
- 最后，Git for Windows 也会自带一套 mingw64 环境，建议换成 #link("https://gitforwindows.org/mingit.html")[MinGit]。
