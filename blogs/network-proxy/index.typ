...
#import theme: template, sidenote, theorem, sidenote
...
#strong[本文仅供技术研究与合规讨论，不构成操作建议或法律建议。读者应自行确认相关行为符合所在地法律法规及服务条款。本文不鼓励任何违法、违规或侵害第三方权益的使用方式]。

== 什么是“网络代理”
#emph[网路代理]是一种转发机制：用户（客户端）将网络请求转发给代理服务器，再由代理去访问目标网站并返回结果。代理的主要作用是修改网络出口，隐藏真实网络行为，以访问受限或不可直连的网络资源。代理也能隔离网络，实现访问控制，不过这里按下不表。
...
网络代理需要搭配#emph[加密网络协议]来隐藏网络行为，否则会被CN大陆各类防火墙设备识别和拦截。虽然 TLS 已经为计算机网络的应用层提供了加密服务，我们仍希望更底层 传输层、网络层 的网络路由信息和行为也被加密保护。因此网络代理协议有专用的 Shadowsocks, VMess, AnyTLS 等协议。

#figure(
  image("./mermaid2.svg", width: 25%)
)
// ```mermaid
// ---
// config:
...
//
//     Server -->|"HTTPS / Other Traffic"| Websites["Websites outside GFW
// Apple, Google, GitHub, etc."]
// ```
...
+ 运行时后台只有一个可执行文件，轻量。可以用 Web 面板管理，也可以用配置文件管理。
+ 封装为自启动服务，在 Windows、Linux 都能无感使用。
+ 配置规则非常方便，直接修改 Mihomo 内核的配置文件即可。
+ 在服务器或家庭网关上配置后，其他内网机器可直接将该机器视为网络代理，使用方便。

迁移到 Mihomo 裸内核的缺点：

+ 学习和配置成本比较高，不能开箱即用，资料比较分散。建议使用 AI 帮助。
+ 不能直接解析订阅链接 URL，需要用 sub-store 转换。这是一个很麻烦的点，下文有解决办法。

== 下载与安装
- 下载符合操作系统要求的 #link("https://github.com/MetaCubeX/mihomo/releases")[`mihomo` 内核]
- 下载控制面板 #link("https://github.com/MetaCubeX/metacubexd")[`MetaCubeX`]，或者直接用#link("https://metacubex.github.io/metacubexd/")[纯网页应用]
- 获取一个机场的订阅链接，用于获取实际可代理上网的节点。
- 按照本文的方法，自定义配置文件 `config.yaml` 。
- 按操作系统要求的方式将 `mihomo` 封装为自启动服务。例如，Linux 用 systemd，Windows 用 WinSW 。

== 配置 Mihomo DNS
推荐本机操作系统的 DNS 服务器配置为 `0.0.0.0:53`，Mihomo 在 `0.0.0.0:53` 端口提供 DNS 代理。 这样 Mihomo 能够提前知晓 ip 和域名的对应关系，关于使用分流规则。否则，应用只请求一个 IP 地址， Mihomo 仍然需要重复请求一次 NS，才能完成域名映射。

Mihomo DNS 有两种增强模式： `redir-host / fake-ip`。在一些情况下，两个不同的域名可能共用 同一个 CDN IP ，这可能导致分流规则应用出错。Mihomo DNS `fake-ip` 模式是为了解决这个问题。 `fake-ip` 还有一个有点，就是应用向 Mihomo DNS 请求 NS 时，可以立即获得结果并返回，不需要 等待实际的 DNS 完成，后续全部代理给 Mihomo。
...
== 配置 Mihomo 代理来源
=== 配置 subconverter
因为 Mihomo 内核直接读取 Proxies 订阅列表，而不是机场提供的订阅 URL，因此需要一个转化工具， 将 URL 转换为 Mihomo 可识别的 YAML 节点列表。常见工具如 Sub-Store 或 subconverter， 这里推荐 #link("https://github.com/MetaCubeX/subconverter")[subconverter]，部署非常轻量。
...
可以将 subconverter 运行在本地，也可以将其持续部署在服务器上，用 nginx 反向代理出接口。 如果在本机部署 subconverter，需要注意转发给 subconverter 的流量也可能被 Mihomo 捕获， 导致部分代理商 URL 走了错误的分流无法访问。（见下文-分流规则）

=== 配置 Mihomo 代理供应商
将 Subconverter 清洗好的节点导入 Mihomo 内核，作为一个 Proxy-Privider。 下面的配置每天向 subconverter 请求一次更新，获取的节点通过 `gstatic` 自动测试与外网的连通性。
...
推荐在 `proxy-providers` 中进行节点测速。Mihomo 文档明确说：#strong[后续 `proxy-groups` url-test 并不会负责 通过 `use: proxy-providers` 引入的网络节点的测速工作。] proxy-groups url-test 只应该负责：拿到 proxy-providers health-check 的测速结果，然后根据延迟进行最优选择。
...
分流规则的格式为：`rult-type,domain,proxy-group`。意思是，在该分流类型下，匹配作用域的所有流量，走指定的代理组。 规则的优先级是从上到下优先匹配。
...
#figure(
  image("./mihomo-dashboard.webp", width: 80%)
)

=== 架构图

#figure(
  image("./mermaid1.svg", width: 70%)
)
...
=== 例子：给 AI 网站配置独立代理链路
假设我们有节点文件 `./proxies/ai.yaml`，将其导入 Mihomo，然后过滤掉不可达的 中国香港/中国澳门 等地区的节点。 每个节点通过 openai 的状态接口来测速，选出最终的优质节点用于访问 ChatGPT 网页。
...
+ 将 #link("https://github.com/winsw/winsw")[WinSW.exe] 重命名为 `mihomo-sc.exe`
+ 配置文件如下，命名为 `mihomo-sc.xml`
+ 用管理员权限进行服务安装和启动：`mihomo-sc.exe install && mihomo-sc.exe start`

这里假设 `mihomo.exe` 内核本体放在 `D:\bin\mihomo` 目录下，配置文件 `config.yaml` 也放在该目录，因此将 工作目录配置为 `D:\bin\mihomo`。 但，按 Windows 实际惯例，应该将 `mihomo.exe` 独立放在某个加入 PATH 的路径，然后工作目录使用 `%LOCALAPPDATA%/mihomo` ，在其中存放所有程序运行时数据。
...
=== 配置局域网的统一代理出口
局域网的服务器、硬件比较多，挨个配置 mihomo 服务确实麻烦。可以将一个机器作为 Proxy ， 实际运行 Mihomo 内核，其他机器通过局域网代理，将流量转发到该 Proxy 即可。
...
注意，#strong[一定要小心安全问题]。绑定所有地址的代价是，所有内网机器都能通过你的机器作为代理。 解决办法：
...
=== 配置本地控制面板
推荐来自 Mihomo 官方的 #link("https://github.com/metacubex/metacubexd")[MetacubeXD]，是一个纯前端的 Web 网页。 不会一直挂在后台，由 Mihomo 内核监听端口并适时拉起。这个端口可以暴露出去，在其他设备访问（注意安全性）。

将打包好的 MetacubeXD 解压到某个目录下，假设是 `E:/metacubexd-ui` 。将面板端口暴露为 `9090`， 此后就可以在浏览器本地访问 `http://127.0.0.1:9090`。
...
=== 调优 (TODO)
TUN 模式: 工作于#link("https://github.com/jay-waves/til/network-l3/IPv4.md")[网络层]. 处理 IP 层及以上的协议.

TAP 模式: 工作于#link("https://github.com/jay-waves/til/data-link-l2.md")[数据链路层], 处理以太网帧, 模拟真实的网卡环境, 如虚拟机组网和桥接网络.

TUN 模式会创建一个虚拟网卡 (utun0), 在网络层就拦截和重定向系统的网络流量.

有几个重要特点:

+ 所有流量 (TCP, UDP, ICMP, DNS) 都会被截获, 因此都会被代理软件的规则 (Rules) 处理
+ 某些应用 (系统更新, 后台服务) 会忽略系统代理, TUN 可以强制接管. 因此其对流量的控制也精准.
+ #strong[hosts 文件和防火墙规则可能被 TUN 模式直接覆盖, 因为流量在网络层就被接管.]
...
== 疑难解答
=== DoH DNS
Mihomo DNS 不要孤立配置使用 `DoH` 服务，比如访问 `https://dns.google.com/dns-query` ，会先触发一次 DNS。 如果这次 DNS 查询没有结果（网络不联通），会造成所有网络访问的解析错误。

至少需要配置一个 fallback udp dns 服务。但是我配置时，表现不太稳定。

=== UWP 应用
UWP 应用主要指 Microsoft Store 以及用它安装的系列应用, 比如 Apple Music, Microsoft Photos 等。 特点是沙盒化，无法访问本地回环，导致挂 Mihomo 代理失败。解决办法是配置一下权限：
...
部分公司，在员工电脑上安装企业根 CA ，使公司网关可以合法伪造 HTTPS 证书，实现 MITM 中间人攻击。 员工的所有 HTTPS 流量都会被公司解密并审查，然后再由公司代理进行外部的网络访问。

因此公司电脑上网时，先检查浏览器网页的证书签发者，是公认权威机构，还是公司自定义 CA。

=== 机场跑路
#strong[近期机场频繁跑路，建议不要长期订阅。详见 #link("https://github.com/limbopro/Paolujichang/issues")[机场跑路贴]]
